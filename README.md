...
控制台主画面是 2×2 四格阶段板，编号即时间顺序 ① 作答 → ② 判定 → ③ 出题 → ④ 自证，
实时显示当前选手所处步骤（黑底=当前/评测中、描边打勾=本轮已完成）；布局上排为选手创作
（①+③）、下排为框架校验（②+④），左列答题链路、右列出题链路。底栏常驻「判定结果」面板
（总判定 PASS/FAIL + 历史/隐藏计数 + 一句结果说明），判定瞬间另有全屏定格 2 秒的录屏镜头。
所有操作按钮点击即执行，无二次确认弹窗。
