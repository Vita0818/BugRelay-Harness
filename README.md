...
控制台主画面是 2×2 四格阶段板，实时显示当前选手所处步骤（黑底=当前/评测中、描边打勾=本轮已完成）：
上排为选手创作一对（① 作答 + ② 出题），下排为框架校验（③ 判定 → ④ 自证；③ 考 ①、④ 考 ②）；
底栏常驻「判定结果」面板（总判定 PASS/FAIL + 历史/隐藏计数 + 一句结果说明），
判定瞬间另有全屏定格 2 秒的录屏镜头。所有操作按钮点击即执行，无二次确认弹窗。

| 步骤 | 谁在做 | 发生什么 |
| ---- | ---- | ---- |
| ① 作答 | 当前选手 | 按需求改 arena_repo 业务代码，交付物落 `inbox/`（或人工上传） |
| ② 出题 | 当前选手 | 交下一棒需求 `next_prompt.md` + 本轮隐藏测试 `hidden_tests.py` |
| ③ 判定 | 框架（人类点「验收答题」） | 应用 ① 的改动到 `src/`，跑历史+隐藏 pytest：全绿过；否则淘汰回滚 |
| ④ 自证 | 框架（人类点「校验出题并交棒」） | 复制仓库 → 验题模型仅凭 ② 的需求单次重实现 → 隐藏测试考它：全绿才交棒，否则出题者淘汰 |

```
① 作答 ─┐                    ┌─ ③ 判定：FAIL 淘汰+回滚，换下一位回到 ①
        ├─（选手创作一对）──→│
② 出题 ─┘                    └─  自证：FAIL 出题者淘汰，换下一位回到 ①
                                  全绿 → 轮次+1 交棒，下一位从 ① 开始
```

Web 操作区对应：答题区（上传 `.zip`/多文件 → 「验收答题」）；出题区（上传需求 + 隐藏测试 → 「校验出题并交棒」）；通用（「还原到最近备份」）。CLI `status` 同样显示当前步骤。
